...
class Graph<E: Hashable>: CustomStringConvertible {
    ...
    func find(from root: E, _ found: (E) -> Bool, budget: Double = Double.greatestFiniteMagnitude) -> E? {
        Graph.path( from: root, find: found, links: { node in
                 self.links.reduce( [ Link ]() ) { links, link in
    ...
    public static func path<S: Sequence>(
            from root: E, find found: (E) -> Bool, neighbours: (E) -> S,
            cost: (E, E) -> Double?, budget: Double = Double.greatestFiniteMagnitude) -> Path?
            where S.Element == E {

        self.path( from: root, find: found, links: { node in
            neighbours( node ).reduce( [ Link ]() ) { neighbours, neighbour in
    ...
    public static func path<S: Sequence>(
            ...
            budget: Double = Double.greatestFiniteMagnitude) -> Path?
            where S.Element == Link {
    ...
    public class Link: Hashable, CustomStringConvertible {
        internal let from:        E
        internal let to:          E
    ...
    public class Path {
        ...
        public init(parent: Path? = nil, target: E, cost: Double) {
            self.parent = parent
            self.target = target
            self.cost = cost
        }
